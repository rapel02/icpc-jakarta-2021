...
def main(argv):
  ...
  M = vr.ReadInt(1, N - 1)
  vr.ReadSpace()
  K, P = vr.ReadIntList(2, 1, N - M)

  C = []
  for _ in range(M):
    c = vr.ReadInt(1, N)
    vr.ReadSpace()
    t = vr.ReadInt(1, 200000)
    vr.ReadEndl()

    C.append(c)

  assert len(C) == len(set(C)), 'Found duplicates in C'

  D = vr.ReadIntList(K, 1, N)
  assert len(D) == len(set(D)), 'Found duplicates in D'
  assert len(set(C).intersection(set(D))) == 0, 'Found intersection in C and D'
