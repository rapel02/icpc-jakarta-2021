...
def main(argv):
  ...
  N = vr.ReadInt(1, 50)
  vr.ReadSpace()
  R, G, P = vr.ReadIntList(3, 1, 1000)
  
  X_Y = []

  for _ in range(N):
    x = vr.ReadInt(-1000, 1000)
    vr.ReadSpace()
    y = vr.ReadInt(-1000, 1000)
    vr.ReadSpace()
    o = vr.ReadInt(0, 270)
    vr.ReadEndl()
    
    assert o % 90 == 0, 'O_i is not a multiple of 90'
    X_Y.append((x, y))

  assert len(X_Y) == len(set(X_Y)), 'Found duplicate in (X, Y)'
