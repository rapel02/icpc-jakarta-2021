...
def main(argv):
  ...
  R, C = vr.ReadIntList(2, 1, 1000)

  assert R * C > 1, 'Field size must bigger than 1'

  token = 0

  for _ in range(R):
    str = vr.ReadString(string.ascii_lowercase + ".", C, C)
    for c in str:
      if c.islower():
        token += 1
    vr.ReadEndl()
  assert token > 0, 'Must have at least 1 token'
  vr.ReadEof()
